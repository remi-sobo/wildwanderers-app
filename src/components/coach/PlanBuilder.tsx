  ...
  return (
    <div className="flex max-w-3xl flex-col gap-6">
      ...
      {workouts.map((w, wi) => (
        ...
        >
          <div className="flex items-center justify-between">
            ...
            {workouts.length > 1 ? (
              <button
                ...
                className="-my-3 -mr-2 flex h-11 w-11 items-center justify-center rounded-lg text-[color:var(--color-text-faint)] transition-colors hover:text-[color:var(--color-state-error)]"
          ...
          <div className="mt-4 flex flex-col gap-2.5">
            {w.exercises.map((ex, ei) => {
              ...
              return (
                ...
                >
                  <div className="relative col-span-3 sm:min-w-[200px] sm:flex-1">
                    <Search
                      size={15}
                      aria-hidden="true"
                    ...
                    />
                    {matches.length > 0 ? (
                      <ul className="absolute left-0 top-[calc(100%+4px)] z-30 max-h-64 w-full min-w-[280px] max-w-[calc(100vw-2rem)] overflow-auto rounded-xl border border-[color:var(--border-strong)] bg-card p-1 shadow-[0_12px_30px_rgba(42,33,24,.14)] sm:w-[320px] sm:max-w-[80vw]">
                        {matches.map((it) => (
                          <li key={it.id}>
                            <button
                              type="button"
                  ...
                  <select
                    ...
                    className={`${fieldClass} col-span-3 w-full capitalize sm:w-[130px]`}
                  >
                    {KINDS.map((k) => (
                  ...
                  <input
                    ...
                    placeholder="Sets"
                    className={`${fieldClass} w-full sm:w-[72px]`}
                  />
                  <input
                    value={ex.reps}
                    onChange={(e) => updateExercise(wi, ei, { reps: e.target.value })}
                    placeholder="Reps"
                    className={`${fieldClass} w-full sm:w-[96px]`}
                  />
                  <input
                    value={ex.load}
                    ...
                    className={`${fieldClass} w-full sm:w-[96px]`}
                  />
                  {ex.mediaUrl || w.exercises.length > 1 ? (
                    <div className="col-span-3 flex items-center justify-end gap-2 sm:contents">
                      ...
                      {w.exercises.length > 1 ? (
                        <button
                          ...
                          className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg text-[color:var(--color-text-faint)] transition-colors hover:text-[color:var(--color-state-error)]"
          ...
          <button
            ...
            className="mt-3 inline-flex items-center gap-1.5 text-[13px] font-semibold text-forest transition-colors hover:text-fern max-md:min-h-[44px]"
          >
      ...
      <button
        ...
        className="inline-flex items-center gap-1.5 self-start rounded-full border border-[color:var(--border-strong)] px-4 py-2 text-[13.5px] font-semibold text-forest transition-colors hover:bg-inset max-md:min-h-[44px]"
      >
      ...
      <div className="flex flex-wrap items-center gap-4 border-t border-[color:var(--border-hair)] pt-6">
        ...
        <button
          ...
          disabled={pending}
          className="inline-flex items-center rounded-full border border-[color:var(--border-strong)] px-5 py-2.5 text-[13.5px] font-semibold text-forest transition-colors hover:bg-inset disabled:opacity-70 max-md:min-h-[44px]"
        >
        ...
        <button
          ...
          disabled={pending}
          className="inline-flex items-center text-[13px] font-semibold text-forest transition-colors hover:text-fern disabled:opacity-70 max-md:min-h-[44px]"
        >
