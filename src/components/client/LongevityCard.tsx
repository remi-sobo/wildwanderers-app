...
import { Activity, ArrowUp, ArrowDown, Minus, Plus, Lock } from "lucide-react";
import {
  recordSelfAssessment,
  grantBodyCompositionConsent,
  revokeBodyCompositionConsent,
} from "@/lib/wellness/actions";
import type { Band, Longevity, LongevityTest } from "@/lib/data/wellness";
...
function BandChip({ band }: { band: Band | null }) {
  if (!band) {
    return (
      <span className="text-[12px] text-[color:var(--color-text-muted)]">Not tested yet</span>
    );
  }
  return (
...
function Trend({ test }: { test: LongevityTest }) {
...
function fmtValue(test: LongevityTest): string | null {
